...
type CredentialCircuit struct {
	...
	// [自动化改造] 使用切片代替定长数组，从而支持外层动态传入树深
	PathElements []frontend.Variable `gnark:",secret"`
...
func (circuit *CredentialCircuit) Define(api frontend.API) error {
	...
	// 动态获取当前测试传入的树深度
	depth := len(circuit.PathElements)
	if depth != len(circuit.PathIndices) {
	...
	// 学术测试：保留底层 MSM 计算，注释掉最后两行以兼容字节对齐
